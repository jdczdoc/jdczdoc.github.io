...
RGB Sensor AVL 位于https://redmine.rockchip.com.cn/projects/rockchip_camera_module_support_list/camera支持列表中显示了sensor模组的详细信息。
...
在Sensor与ISP之间MIPI通讯需要设置4个参数，请**务必**确认4个MIPI参数的正确性。
...
- 使用gstreamer的v4l2src plugin可以从/dev/video设备中获取图像并**显示在屏幕**上
  ...
  **特别需要注意:** RK先后提供过多个版本的gstreamer isp plugin，如rkisp, rkv4l2src，都已经**不再继续支持**。请直接使用gstreamer自带的**v4l2src** plugin。主要有两点原因：
