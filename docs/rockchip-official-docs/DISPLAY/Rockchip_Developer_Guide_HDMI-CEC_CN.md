...
| rx_status        | CEC 驱动完成接收 MSG 后将会对其设置，标记接收的状态  CEC_RX_STATUS_OK CEC_RX_STATUS_TIMEOUT CEC_RX_STATUS_FEATURE_ABORT |
| tx_status        | CEC 驱动完成发送 MSG 后将会对其设置，标记发送的状态: CEC_TX_STATUS_OK CEC_TX_STATUS_ARB_LOST CEC_TX_STATUS_NACK CEC_TX_STATUS_LOW_DRIVE CEC_TX_STATUS_ERROR CEC_TX_STATUS_MAX_RETRIES |
...
| configuring                                | 1：正在配置 CEC 0：尚未开始配置 CEC 或已经配置完成        |
