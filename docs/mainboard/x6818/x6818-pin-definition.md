...
sidebar_position: 4
title: 引脚定义
description: X6818 核心板和扩展接口完整引脚定义
---

# 引脚定义

X6818CV3 核心板向下兼容 X4418CV3，核心板管脚定义相同。下表保留硬件手册中的核心板引脚和扩展接口定义，适合底板设计、飞线验证、接口调试和设备树复用检查。

:::note
X6818 接口资源较多，LCD、LVDS、MIPI、Camera、UART、SPI、ADC、GPIO 等存在复用或板级连接关系。修改底板或设备树前，应先确认硬件接口和引脚复用是否冲突。
:::
