...
sidebar_position: 7
title: Android 编译与烧录
description: X3566 Android 11 开发环境、源码编译、镜像输出和烧录流程
---

# Android 编译与烧录

本页整理 X3566 Android 平台开发环境、源码安装、编译和烧录流程。
...
Android 编译需要根据 SDK 脚本选择 JDK。X3566 Android 11 常见配置可使用 Java 8：
...
X3566 Android 平台中，PDF 版手册给出的路径如下：
...
:::note
上传的 Word 版 Android 手册中存在一处明显从其他平台继承的路径写法：`rk3568-evb1-ddr4-v10.dts`。本模板按 X3566 Android PDF 中的 `rk3566-rk817-tablet.dts` 作为 X3566 页面默认路径。
:::
...
如果是分卷压缩包，需要在 `x3566_Android 11` 目录下继续解压 `prebuilts.tar.bz2`。
...
4. 让开发板进入 Loader / Maskrom / Recovery 相关烧录模式。
5. 点击升级并等待完成。
